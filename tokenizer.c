...
word2[j2++] = '/';
word2[j2] = '\0';
s2[n2] = malloc(sizeof(char) * _strlen(word2) + 1);
_strcpy(s2[n2], word2);
j2 = 0;
n2++;
memset(word2, 0, sizeof(char) * _strlen(word2) + 1); }
else
word2[j2++] = string2[i2];
i2++; }
s2[n2] = malloc(sizeof(char) * _strlen(word2) + 1);
...
char *seperators = malloc(_strlen(sep) + 1);
...
{
buf[l] = '\0';
allocation_size = sizeof(char) * (_strlen(buf) + 1);
